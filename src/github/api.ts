...
export function createGitHubClient(token: string): GitHubClient {
	...
	return {
		...
		async getCommitStatuses(owner, repo, ref) {
			return request<GitHubCommitStatus[]>('GET', `/repos/${owner}/${repo}/commits/${ref}/statuses?per_page=100`);
		},
		...
		async getRepoCustomProperties(owner, repo) {
			try {
				return await request<GitHubCustomProperty[]>('GET', `/repos/${owner}/${repo}/properties/values`);
			} catch {
				return [];
			}
		},
