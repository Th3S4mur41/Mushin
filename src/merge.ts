...
import { buildUnknownTypeMessage, upsertStatusComment } from './comment';
 ...
 */
async function areChecksPassing(client: GitHubClient, owner: string, repo: string, sha: string): Promise<boolean> {
...
export async function evaluatePullRequest(
	...
	const dependencies = await parse(commitMessage, body, pr.head.ref, pr.base.ref);
	...
	if (highestType === undefined) {
		...
		console.log(
			`[mushin] PR ${owner}/${repoName}#${pr.number}: merging (${highestType} ≤ ${config.highestVersionToMerge})`,
		);
	}
