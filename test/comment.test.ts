...
import { upsertStatusComment, buildUnknownTypeMessage } from '../src/comment';
...
describe('upsertStatusComment', () => {
  ...
  it('updates existing comment when content changes', async () => {
    ...
    const client = makeClient({
      getPRComments: vi.fn().mockResolvedValue([
        { id: 42, body: existingBody, user: { login: 'mushin[bot]' } },
      ]),
    });
  ...
  it('ignores comments from other users', async () => {
    ...
    const client = makeClient({
      getPRComments: vi.fn().mockResolvedValue([
        { id: 42, body: existingBody, user: { login: 'other-user' } },
      ]),
    });
