...
describe('resolveConfig', () => {
	...
	it('reads highest_version_to_merge from custom properties', async () => {
		const client = makeClient({
			getRepoCustomProperties: vi
				.fn()
				.mockResolvedValue([{ property_name: 'mushin_highest_version_to_merge', value: 'major' }]),
		});
		const config = await resolveConfig(client, 'owner', 'repo', []);
	...
	it('reads merge_unknown from custom properties', async () => {
		const client = makeClient({
			getRepoCustomProperties: vi.fn().mockResolvedValue([{ property_name: 'mushin_merge_unknown', value: 'true' }]),
		});
		const config = await resolveConfig(client, 'owner', 'repo', []);
	...
	it('labels override repo YAML and custom properties', async () => {
		const client = makeClient({
			getRepoCustomProperties: vi
				.fn()
				.mockResolvedValue([{ property_name: 'mushin_highest_version_to_merge', value: 'patch' }]),
			getFileContent: vi.fn().mockResolvedValue('highest_version_to_merge: minor'),
		});
		const config = await resolveConfig(client, 'owner', 'repo', [{ name: 'mushin:merge-major' }]);
		expect(config.highestVersionToMerge).toBe('major');
	});
	...
	it('ignores invalid values in custom properties', async () => {
		const client = makeClient({
			getRepoCustomProperties: vi
				.fn()
				.mockResolvedValue([{ property_name: 'mushin_highest_version_to_merge', value: 'invalid' }]),
		});
		const config = await resolveConfig(client, 'owner', 'repo', []);
